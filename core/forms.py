...
from .models import Sala, DIAS_SEMANA, Agendamento
        ...
        for day_code, day_label in DAYS_OF_WEEK:
            self.fields[f"{day_code}_from"] = forms.IntegerField(
                ...
                widget=forms.NumberInput(
                    attrs={"placeholder": "e.g. 9", "class": "form-control"}
                ),
            )
            self.fields[f"{day_code}_to"] = forms.IntegerField(
                ...
                max_value=24,
                widget=forms.NumberInput(
                    attrs={"placeholder": "e.g. 17", "class": "form-control"}
                ),
            )
            ...
            if (
                ...
                and self.instance.horarios_disponiveis
            ):
                    ...
                    if isinstance(parsed, list):
                        for slot in parsed:
                            if (
                                ...
                                day_slots.append(
                                    {"from": int(slot["from"]), "to": int(slot["to"])}
                                )
                except Exception:
                    pass
            ...
            if not day_slots:
                if from_val is not None and to_val is not None:
                    if from_val >= to_val:
                        self.add_error(
                            f"{day_code}_to",
                            f"'{day_code}' end hour ('to') must be greater than start hour ('from').",
                        )
                    else:
                        day_slots.append({"from": int(from_val), "to": int(to_val)})
                elif from_val is not None and to_val is None:
                    self.add_error(
                        f"{day_code}_to", f"Please enter 'to' hour for {day_code}."
                    )
                elif from_val is None and to_val is not None:
                    self.add_error(
                        f"{day_code}_from", f"Please enter 'from' hour for {day_code}."
                    )
...
class AgendamentoSearchForm(forms.Form):
    horas_seguidas = forms.IntegerField(
        min_value=1,
        max_value=24,
        label="Quantidade de horas seguidas",
        widget=forms.NumberInput(
